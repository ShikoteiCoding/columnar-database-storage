...
class DataTableQuestionTests(unittest.TestCase):
    ...
    def test_append_rows_spills_overflow_into_later_row_groups(self) -> None:
        ...
        self.assertEqual(
            [
                (row_group.start, row_group.count)
                for row_group in table.row_groups.row_groups.nodes
            ],
            [(0, 3), (3, 3)],
        )
        self.assertEqual(
    ...
    def test_append_rows_raises_for_invalid_rows_without_mutating_existing_data(
        self,
    ) -> None:
        table = self.make_table(row_group_size=2)
        table.append_rows(
